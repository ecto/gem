...
async function main() {
  ...
  while (true) {
    ...
    const action = await runEngine(theta, memory.getRecent(), observation);
    // console.log("Action selected:", JSON.stringify(action));
    ...
    switch (action.type) {
      ...
      case 'sys':
        ...
        if (result.committed) {
          theta = result.theta;
          saveTheta(theta);
          console.log(`[SYS COMMIT] J improved from ${result.jOld.toFixed(2)} to ${result.jNew.toFixed(2)}`);
          nextObservation = "Configuration updated successfully.";
        } else {
          console.log(`[SYS ROLLBACK] J failed to improve (${result.jOld.toFixed(2)} -> ${result.jNew.toFixed(2)})`);
